...
Повторный парсинг не выполняется для неизменённых файлов: индексатор сравнивает `file_hash` с сохранённым в `index_state` и пропускает файл при совпадении хэша. Отдельного кэша текста нет — чанки уже хранятся в ChromaDB.
...
3. Чанкинг: ~1000 символов, перекрытие ~100 (посимвольно).
...
2. **Авто-индексация:** файлы со статусом отличным от `indexed` (`stale`, `error`, новые) индексируются перед анализом, чтобы иметь актуальные векторы.
3. **Получение векторов:** для каждого файла переиспользуются эмбеддинги из ChromaDB (среднее векторов чанков). Повторного обращения к Ollama не происходит.
4. **Кластеризация** документных векторов (scikit-learn KMeans; число кластеров k подбирается автоматически по silhouette score в диапазоне 2..min(8, n-1)). Файлы без векторов помечаются как некластеризуемые.
5. **Именование кластеров:** для каждого кластера выбираются репрезентативные чанки → один вызов `qwen3:1.7b` на кластер для генерации имени папки. Fallback без LLM: имя из топ TF-IDF ключевых слов.
6. **Папка «Разное»:** файлы без векторов помещаются туда.
7. **Сборка превью** → возврат в UI.
8. **Применение (по подтверждению):** FS Service создаёт папки и перемещает файлы. Имена папок проверяются на path traversal. До этого шага диск не изменяется.
...
- **Название:** генерируется из первого вопроса пользователя (усечение до 60 символов); редактируется пользователем в любой момент.
...
| thinking | text | Рассуждения модели (reasoning), если были |
...
|---|---|---|
| id | PK | Идентификатор |
| file_path | text (unique) | Путь к файлу |
| file_hash | text | Хэш содержимого (для инкрементальности) |
| status | text | `indexed` / `stale` / `error` |
| error_msg | text | Текст ошибки (для статуса `error`) |
| mtime | float | Время изменения (быстрый префильтр изменений) |
| size | int | Размер файла (быстрый префильтр изменений) |
