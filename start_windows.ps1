...
function Get-WindowsVenvPython {
    param(
...
function Test-UnixStyleVenv {
    ...
    return (
        (Test-Path (Join-Path $VenvDir 'bin\python')) -or
        (Test-Path (Join-Path $VenvDir 'bin\python.exe'))
    )
}
...
$VenvPython = Get-WindowsVenvPython -VenvDir $VenvDir

if ($VenvPython -and $SkipInstall) {
    Write-Warn 'Skipping dependency install (-SkipInstall)'
    Pop-Location
} else {
    if ((Test-Path $VenvDir) -and (-not $VenvPython)) {
        if (Test-UnixStyleVenv -VenvDir $VenvDir) {
            Write-Warn 'Detected Unix-style .venv (bin/...). Recreating Windows venv...'
        } else {
            Write-Warn '.venv exists, but Scripts\python.exe is missing. Recreating Windows venv...'
        }
    ...
    $VenvPython = Get-WindowsVenvPython -VenvDir $VenvDir
    if (-not $VenvPython) {
        Write-Fail 'Windows venv is invalid: expected .venv\Scripts\python.exe'
        Pop-Location ; exit 1
    }
